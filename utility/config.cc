	...
	while (!feof(cfg))
	{
		...
		if ((ligne[0] != '#') && (strlen(ligne) != 0))
		{
			...
			if (strlen(commande) != 0)
			{
				...
				if (strcmp(commande, "NbProgUserArg") == 0)
				{
					if(sscanf(ligne, "%s = %i ",commande,&UserProgNbArg) != 2)
						fail(nblignes,configname,ligne);
					continue;
				}

				if (strcmp(commande, "UserProgArg") == 0)
				{
					if(this->UserProgNbArg == 0)
						this->UserProgNbArg++; //at least the prog name
					UserProgArgs = (void**)malloc(sizeof(void*)*this->UserProgNbArg);
					UserProgArgs[0] = malloc(sizeof(char)*(strlen(ProgramToRun)+1));
					strcpy((char*)UserProgArgs[0],this->ProgramToRun);
					printf("w  t f %s\n",UserProgArgs[0]);
					DEBUG('u',(char*)"%d arg : %s\n",this->UserProgNbArg,this->UserProgArgs[0]);
					int i;
					char * t = ligne;
					char *arg_holder;
					// UserProgArgs = arg1 arg2 
					//this loop move the pointer from the beginning to after =
					...
					int arg_len;
					i = 1;
					while((arg_holder = strsep(&t," \t")) != NULL) {
						arg_len = strlen(arg_holder)+1;
						UserProgArgs[i] = malloc(sizeof(char)*arg_len);
						strcpy((char*)UserProgArgs[i],arg_holder);
						i++;
					}
					continue;
				}
