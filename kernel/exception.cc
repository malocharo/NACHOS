...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr)
{
  ...
  case SYSCALL_EXCEPTION: {
	...
	case SC_LOCK_CREATE: {
		...
		uint32_t lockAddr = g_machine->ReadIntRegister(4); //read r4 first param
		if(lockAddr) { //can be zero,wich show it's an error
			int sizeParam = GetLengthParam(lockAddr);
			char name[sizeParam];
			GetStringParam(lockAddr,name,sizeParam);

			Lock * lock = new Lock(name);
			int32_t objId = g_object_ids->AddObject(lock);
			g_machine->WriteIntRegister(2,objId);
			g_syscall_error->SetMsg((char *)"no error\n",NO_ERROR);
		} else {
			g_machine->WriteIntRegister(2,ERROR);
			g_syscall_error->SetMsg((char *)"Invalid lock ID\n",INVALID_LOCK_ID);
		}
		break;
	...
	case SC_LOCK_ACQUIRE: {
		uint32_t lockAc = g_machine->ReadIntRegister(4);
		Lock* lock = (Lock*) g_object_ids->SearchObject(lockAc);
	...
	case SC_LOCK_DESTROY: {
		...
		Lock * lock = (Lock*) g_object_ids->SearchObject(lockAc);
		if(lock && lock->type == LOCK_TYPE) {
			g_object_ids->RemoveObject(lockAc);
			delete lock;
			g_machine->WriteIntRegister(2,NO_ERROR);
			g_syscall_error->SetMsg((char*)"no error\n",NO_ERROR);
		} else {
			g_machine->WriteIntRegister(2,ERROR);
			g_syscall_error->SetMsg((char*)"invalid id lock\n",INVALID_LOCK_ID);
		}
		break;
	}
